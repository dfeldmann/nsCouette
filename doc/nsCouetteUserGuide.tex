...
examples of how to correctly configure and build some of the required libraries (\code{FFTW3}, \code{zlib} and \hdf) for the use with \nsc on our local \code{fsmcluster} at the  
...
\mod{The radial distribution of grid points can be specified at runtime by modifying the
...
\par
\mod{Alternatively, the radial grid can be read from an external file. This option is
...
\par
\mod{When the simulation is started from a checkpoint file (see section~\ref{sec:checkpoint}),
...
\par
In this example, a number of \code{numsteps=10000} time steps would be attempted
...
\begin{figure}[htb]
\centering
\includegraphics[width=1.00\columnwidth]{../paper/figures/1nscouetteBenchmark.pdf}
...
\mod{As described in detail in section~\ref{sec:spatialDiscretisation}, \nsc is based
...
\par
\mod{Because the physical flow field ($\vec{u}, p, T$) is real valued,
...
\par
\mod{Note that to keep the matrices banded the stencil length decreases towards 
...
\par
\mod{As a final remark, we would like to stress that increasing the order of the
...
\mod{Data distribution across \mpi tasks is accomplished using a one-dimensional (1d)
...
\par
\mod{Since the computation of the nonlinear terms is carried out using a pseudospectral
...
transposes.}
\par
\mod{Note that both the number of radial modes $(m\_r)$ and Fourier modes $(m\_f)$ must
...
by $np$.}
\par
\mod{Since in turbulent simulations the number of radial modes is typically much smaller
...
\par
\mod{A sketch illustrating the data distribution for this hybrid strategy is shown in
...
\label{fig:domdec}
\end{figure}
\mod{Data chunks resulting from the \mpi slab decomposition are indicated by the
...
\end{enumerate}}
\mod{Finally, we would like to note that an alternative strategy to the use of \omp is
...
of the simulation, \mod{see section~\ref{sec:spatialDiscretisation}}. Plots
like these in figures~\ref{fig:tc0040keThZ}, \ref{fig:tc0041keThZ} and
\ref{fig:tc0042keThZ} become very handy as a first indicator for the quality of the spatial resolution. \mod{To
...
\end{figure}
\mod{Since momentum and energy must be conserved, a minimal requirement
...
\begin{figure}[htb]
\centering
\includegraphics[width=1.00\textwidth]{../paper/figures/3compareGr.pdf}
...
\par
\mod{As already mentioned in the tutorials before (e.g. \ref{sec:tc0041}),
...
\par
\mod{Here, we will only briefly discuss those details that are specific to \nsp,
...
\mod{We consider a fluid flowing through a straight circular pipe of constant
...
\par
\mod{The term $F\hat{e}_z$ in eq.~\eqref{eq:eq_pipe} denotes the volumetric
