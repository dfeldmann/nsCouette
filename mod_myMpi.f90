...
MODULE mod_myMpi
  ...
  INTEGER(KIND=4), PRIVATE :: block_lengths(2),types(2),extent
...
CONTAINS
  ...
  subroutine init_mpidist
    ...
    IF ((MOD(m_r,numprocs) /= 0)) THEN
       PRINT*, 'm_r is  not divisible by number of processors.'
    ...
    if ((MOD(m_f,numprocs) /= 0)) then
       ...
       if (myid==root) then
          ...
          print '(A,I9,A,I5,A,I8,A,I8,A)', '   => m_f grid enlarged to',m_f,', populated with',&
               &(np-1),'*',mp_fmax,'+',(m_th+1)*m_z-(np-1)*mp_fmax,' modes'
          print '(A)',                     '------------------------------------------------------------------------'
       endif
    else
    ...
    !identify number of modes (kth,0)

    k0_modes = 0

    do i=1,mp_f
       if ((i+myid*mp_f) .le. m_th+1) then
          k0_modes = k0_modes + 1
       end if
    end do

    print*,'myid.k0modes',myid,k0_modes
  ...
  subroutine xTranspose_mpi(m,n,in,out,ifile)
    ...
    integer(KIND=4) :: shape_i(2),shape_o(2),i
    logical :: extend
