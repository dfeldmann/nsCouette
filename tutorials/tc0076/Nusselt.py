#!/usr/bin/env python3
#
# Purpose:  read torque time series from ascii file, compute average, plot
#           everything, print difference between inner and outer torque
...
#ax1.set_title(r"Temporal evolution of Nusselt, $Re_i=140$, \texttt{tc0075}")
plt.setp(ax1.get_xticklabels(), visible=False)
ax1.set_ylabel(r"$N\!u$")
ax1.plot(t,   ti,   color='blue', linestyle='-',  label=r"Inner")
ax1.plot(tap, timp, color='black', linestyle='--', label=r"$\langle N\!u_{i} \rangle_{t_a}$")
ax1.plot(t,   to,   color='red',  linestyle='-',  label=r"Outer")
ax1.plot(tap, tomp, color='black', linestyle=':', label=r"$\langle N\!u_{o} \rangle_{t_a}$")
...
ax2.set_ylabel(r"$N\!u$")
plt.setp(ax2.get_xticklabels(), visible=False)
ax2.plot(ta, tif, color='blue', linestyle='-', label=r"$N\!u_{i}^{\prime}=N\!u_{i}-\langle N\!u_{i} \rangle_{t_a}$")
ax2.legend(loc='best', ncol=1)

# plot outer torque fluctuation
ax3 = plt.subplot2grid((3, 1), (2, 0), rowspan=1, colspan=1, sharex=ax1)
ax3.set_ylabel(r"$N\!u$")
ax3.set_xlabel(r"$t$ in $\sfrac{d}{\nu^{2}}$")
ax3.plot(ta, tof, color='red', linestyle='-', label=r"$N\!u_{o}^{\prime}=N\!u_{o}-\langle N\!u_{o} \rangle_{t_a}$")
